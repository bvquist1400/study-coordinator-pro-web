...
export async function PUT(
  ...
  // Require aggregated per-drug cycles path
  if (requestData.cycles || requestData.adjustments) {
      ...
      return NextResponse.json({ error: 'cycles array is required. Legacy single-bottle formats are no longer supported.' }, { status: 400 })
    }
