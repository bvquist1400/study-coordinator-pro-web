...
function LegacyLabKitsPageLoader() {
  return (
    <DashboardLayout>
      <div>
...
export default function LegacyLabKitsPage() {
  return (
    <Suspense fallback={<LegacyLabKitsPageLoader />}>
      <LegacyLabKitsPageContent />
    </Suspense>
  )
}
...
const LEGACY_ALLOWED_VIEWS: ViewMode[] = ['inventory', 'forecast', 'orders-shipments', 'archive', 'alerts', 'settings']

function LegacyLabKitsPageContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const initialStudyIdParam = searchParams.get('studyId')
  const [studies, setStudies] = useState<Study[]>([])
  const deriveInitialView = (): ViewMode => {
    const viewParam = (searchParams.get('view') ?? '').toLowerCase()
    return LEGACY_ALLOWED_VIEWS.includes(viewParam as ViewMode) ? (viewParam as ViewMode) : 'inventory'
  }
  const [selectedStudyId, setSelectedStudyId] = useState<string>(() => initialStudyIdParam || 'all')
  const [viewMode, setViewMode] = useState<ViewMode>(deriveInitialView)
  ...
  const [ordersNotice, setOrdersNotice] = useState<{ type: 'success' | 'error' | 'info'; message: string } | null>(null)
  ...
  const [pendingOrderPrefillId, setPendingOrderPrefillId] = useState<string | null>(null)
  const [orderModalDefaults, setOrderModalDefaults] = useState<{ kitTypeId?: string | null; kitTypeName?: string | null; quantity?: number | null } | null>(null)
  const [forecastSuggestions, setForecastSuggestions] = useState<Array<{ kitTypeId: string | null; kitTypeName: string; quantity: number }>>([])
  ...
  const clearActionParams = useCallback(() => {
    ...
    router.replace(`/lab-kits/legacy${query ? `?${query}` : ''}`, { scroll: false })
  }, [router, searchParams])
  ...
  useEffect(() => {
    ...
    const viewParam = (searchParams.get('view') ?? '').toLowerCase()
    if (LEGACY_ALLOWED_VIEWS.includes(viewParam as ViewMode) && viewParam !== viewMode) {
      setViewMode(viewParam as ViewMode)
    }
    const actionParam = searchParams.get('action')
    const orderIdParam = searchParams.get('orderId')
    if (actionParam && actionParam !== lastActionRef.current) {
      lastActionRef.current = actionParam
      switch (actionParam) {
        case 'start-order': {
          setViewMode('orders-shipments')
          setOrderModalDefaults(null)
        ...
        case 'plan-order': {
          setViewMode('orders-shipments')
          const kitTypeIdParam = searchParams.get('kitTypeId')
          const kitTypeNameParam = searchParams.get('kitTypeName')
          const quantityParam = searchParams.get('quantity')
          const parsedQuantity = quantityParam ? Number.parseInt(quantityParam, 10) : NaN
          const resolvedQuantity = Number.isFinite(parsedQuantity) && parsedQuantity > 0 ? parsedQuantity : null
          setOrderModalDefaults({
            kitTypeId: kitTypeIdParam,
            kitTypeName: kitTypeNameParam,
            quantity: resolvedQuantity
          })
          setOrdersNotice({ type: 'info', message: 'Forecast recommendation prefilled. Adjust as needed before submitting.' })
          setShowOrderModal(true)
          clearActionParams()
          break
        }
        case 'log-receipt': {
          setViewMode('orders-shipments')
          if (orderIdParam) {
            setPendingOrderPrefillId(orderIdParam)
            setOrdersNotice({ type: 'info', message: 'Prefilling from pending order.' })
          } else {
            setOrdersNotice({ type: 'success', message: 'Locate the order below to mark it as received.' })
            clearActionParams()
          }
          break
        }
  ...
  }, [searchParams, selectedStudyId, viewMode, clearActionParams])

  useEffect(() => {
    if (!pendingOrderPrefillId) return
    if (!selectedStudyId || selectedStudyId === 'all') {
      setPendingOrderPrefillId(null)
      setOrdersNotice({ type: 'error', message: 'Select a specific study to log order receipts.' })
      clearActionParams()
      return
    }
    ...
    const loadOrderPrefill = async () => {
      try {
        ...
        const response = await fetch(`/api/lab-kit-orders?studyId=${selectedStudyId}`, {
        ...
        const target = rows.find((row: any) => row.id === pendingOrderPrefillId)
        if (!target || cancelled) {
          setOrdersNotice({ type: 'error', message: 'Unable to find the selected order. Please mark it manually.' })
          return
        }

        const receivedDate = todayLocalISODate()

        const patchResponse = await fetch(`/api/lab-kit-orders/${pendingOrderPrefillId}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`
          },
          body: JSON.stringify({ status: 'received', receivedDate })
        })
        ...
        const resolvedOrder = patched?.order ?? target

        setOrdersNotice({ type: 'success', message: 'Order marked as received. Inventory entry is prefilled below.' })
        setOrdersRefreshKey(prev => prev + 1)
        setAddPrefill({
          kitTypeId: resolvedOrder?.kit_type_id ?? null,
          kitTypeName: resolvedOrder?.kit_type_name ?? null,
          receivedDate,
          quantity: typeof resolvedOrder?.quantity === 'number' ? resolvedOrder.quantity : null
        })
        setShowAddModal(true)
        setViewMode('inventory')
      } catch (error) {
        if (!cancelled) {
          setOrdersNotice({ type: 'error', message: error instanceof Error ? error.message : 'Failed to load order details.' })
        }
      } finally {
        if (!cancelled) {
          setPendingOrderPrefillId(null)
  ...
  }, [pendingOrderPrefillId, selectedStudyId, clearActionParams])
  ...
  const updateQueryParams = useCallback((next: { studyId?: string | null }) => {
    ...
    router.replace(`/lab-kits/legacy${queryString ? `?${queryString}` : ''}`)
  }, [router, searchParams])
  ...
  if (loading) {
    return <LegacyLabKitsPageLoader />
  }
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-white">Lab Kit Management Workspace</h1>
              <p className="text-gray-400 mt-1">Tabbed interface for detailed inventory management, forecasting, and shipments.</p>
            </div>
            <div className="flex flex-wrap items-center justify-start gap-2 md:justify-end">
              ...
              <button
                onClick={() => {
                  if (!canManageStudy) return
                  if (viewMode === 'forecast' && forecastSuggestions.length > 0) {
                    const topSuggestion = forecastSuggestions[0]
                    setOrderModalDefaults({
                      kitTypeId: topSuggestion.kitTypeId,
                      kitTypeName: topSuggestion.kitTypeName,
                      quantity: topSuggestion.quantity
                    })
                    setOrdersNotice({ type: 'info', message: 'Forecast recommendation prefilled. Adjust as needed before submitting.' })
                  } else {
                    setOrderModalDefaults(null)
              ...
              <Link
                href={
                  selectedStudyId && selectedStudyId !== 'all'
                    ? `/lab-kits?studyId=${selectedStudyId}`
                    : '/lab-kits'
                }
                className="rounded-lg border border-blue-500/50 px-4 py-2 text-sm font-semibold text-blue-200 transition-colors hover:border-blue-400 hover:text-blue-100"
              >
                Unified dashboard
              </Link>
            ...
            {viewMode === 'forecast' && (
              selectedStudyId === 'all' ? (
                ...
                <InventoryForecast
                  studyId={selectedStudyId}
                  daysAhead={30}
                  onSuggestedOrdersChange={setForecastSuggestions}
                />
              )
            )}
        ...
        {showOrderModal && canManageStudy && (
          <LabKitOrderModal
            ...
            defaultKitTypeId={orderModalDefaults?.kitTypeId ?? undefined}
            defaultKitTypeName={orderModalDefaults?.kitTypeName ?? undefined}
            defaultQuantity={orderModalDefaults?.quantity ?? undefined}
            allowKitTypeChange
          />
        )}
      </div>
